...
vTensor pack_weights(
  ...
  {
    uint32_t numel = KH * KW * 4 * OC_4 * 4 * C_4;
    int bufferSize = OC_4 * C_4 * 16 * KH * KW;
      ...
      for (uint32_t sy = 0; sy < OC_4; ++sy) {
        for (uint32_t sz = 0; sz < (KH * KW); ++sz) {
          for (uint32_t vi = 0; vi < 4; ++vi) {
            ...
            image[4 * sx + 0][sy][sz][vi] = buffer[4 * (bufferVIdx + 0) + vi];
            image[4 * sx + 1][sy][sz][vi] = buffer[4 * (bufferVIdx + 1) + vi];
            image[4 * sx + 2][sy][sz][vi] = buffer[4 * (bufferVIdx + 2) + vi];
            image[4 * sx + 3][sy][sz][vi] = buffer[4 * (bufferVIdx + 3) + vi];
          }
        }
      }
    }
